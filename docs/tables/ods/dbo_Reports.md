...
Центральная таблица процесса: содержит основную информацию по заявкам (проверкам аналитиков), из неё берутся данные для большинства расчётов и показателей проекта. _TODO: дополнить детальным описанием того, что представляет собой одна запись/заявка._

## Связи с другими таблицами

- `Analytic` → `dbo.Employees.Email` — сотрудник (аналитик), выполнивший заявку.
- `Subdivision` → `dbo.camera_directory.CameraSubdivision` — точка/АЗС, к которой относится заявка. _TODO: проверить, не является ли более надёжным ключом связи `SubdivisionId` (есть в обеих таблицах)._
